...
const TrashBinAddresses = () => {
    const [addresses, setAddresses] = useState<TrashBinAddress[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [editingAddress, setEditingAddress] = useState<TrashBinAddress | null>(null);
    const [formData, setFormData] = useState<Partial<TrashBinAddress>>({
        address_id: '',
        label: '',
        address: '',
        lat: undefined,
    ...
    const fetchAddresses = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_URL}/trash-bin-addresses`, {
                headers: {
    ...
    const handleOpenModal = (address?: TrashBinAddress) => {
        if (address) {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const uploadPromises = Array.from(files).map(async (file) => {
                ...
                const response = await fetch(`${API_URL}/upload/trash-bin-image`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    },
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = editingAddress
            ? `${API_URL}/trash-bin-addresses/${editingAddress.address_id}`
            : `${API_URL}/trash-bin-addresses`;
        ...
        try {
            ...
            console.error('Error saving trash bin address:', error);
            alert('An error occurred while saving.');
        }
    };

    const handleDelete = async (id: string) => {
        if (!window.confirm('Are you sure you want to delete this trash bin address?')) return;

        try {
            const response = await fetch(`${API_URL}/trash-bin-addresses/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    return (
        <div className={styles.recyclingPage}>
            <div className="header">
                <div>
                    <h1>Trash Bin Addresses</h1>
                    <p className="subtitle">Manage public trash bin locations and collection points</p>
                </div>
            </div>

            <div className={styles.mgmtCard}>
                <div className={styles.cardHeaderWithSearch}>
                    <h2 className={styles.cardTitle}>Trash Bin List</h2>
                ...
                <div className={styles.tableContainer}>
                    {loading ? (
                        ...
                        <div className={styles.emptyState}>
                            <TrashIcon size={48} />
                            <p>No trash bin addresses found</p>
                        </div>
                    ) : (
                ...
                <div className={styles.modalOverlay}>
                    <div className={styles.modal}>
                        <div className={styles.modalHeader}>
                            <h2>{editingAddress ? 'Edit Trash Bin' : 'Add New Trash Bin'}</h2>
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className={styles.formGrid}>
                                ...
                                <div className={`${styles.formGroup} ${styles.fullWidth}`}>
                                    <label>Bin Name / Label *</label>
                                    <input
                                        type="text"
                                        name="label"
                                        value={formData.label}
                                        onChange={handleInputChange}
                                        placeholder="e.g. Public Bin Soi 5"
                                        required
                                    />
                                </div>
                                <div className={`${styles.formGroup} ${styles.fullWidth}`}>
                                ...
                                <div className={`${styles.formGroup} ${styles.fullWidth}`}>
                                    <label>Trash Bin Images</label>
                                    <div className={styles.imageUploadSection}>
                                        <div className={styles.imageGrid}>
                                            {(formData.images || []).map((url, index) => (
                                                <div key={index} className={styles.imagePreview}>
                                                    <img src={url} alt={`Bin ${index}`} />
                            ...
                            <div className={styles.modalFooter}>
                                ...
                                <button type="submit" className={styles.saveBtn} disabled={uploading}>
                                    {editingAddress ? 'Update Trash Bin' : 'Create Trash Bin'}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default TrashBinAddresses;
